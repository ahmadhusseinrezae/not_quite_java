...
public class FileTranslationTest {
	...
	public static Collection<Object[]> data() {
		...
		// under this directory, i have added a directory called additional which
		// has test cases i have created. they include both type checking and translation
		// concerns.
		...
		Arrays.stream(Objects.requireNonNull(new File("testdata/typechecker/ok")
                .listFiles(File::isDirectory)))
                .forEach(file -> appendTestCasesFromDir(file, ctorParams)
        );

		// The test cases in below directory have been tested with typechecker tester
		// but since they are supposed to failed i have comented them out. 
		// Arrays.stream(Objects.requireNonNull(new File("testdata/typechecker/error")
        //         .listFiles(File::isDirectory)))
        //         .forEach(file -> appendTestCasesFromDir(file, ctorParams)
        // );
