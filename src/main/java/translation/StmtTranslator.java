...
public class StmtTranslator implements NQJStatement.MatcherVoid {
    ...
    public void case_StmtAssign(NQJStmtAssign s) {
        ...
        boolean isArray = Helper.isItArray(s.getAddress());
        // then translate the right hand side
        final Operand rValue = tr.exprRvalue(s.getValue());

        boolean isNewObject = Helper.isNewObject(s.getValue());

        if (isArray && isNewObject) {
            tr.addInstruction(Ast.Store(lAddr, rValue)); // objects are saved as a pointer inside array
        } else {
            final Operand rValueCasted =
                    tr.addCastIfNecessary(rValue, ((TypePointer) lAddr.calculateType()).getTo());
